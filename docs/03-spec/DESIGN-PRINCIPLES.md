...
| 8 | **Craft** (high-quality materials: fonts, **light+dark adaptation**, icons, **responsive animation w/ immediate feedback**, no jank) | Inter/Inter Display, Spatial Liquid Glass, spring press, scroll-reveal, focus-visible rings, global reduced-motion guard, **full `prefers-color-scheme` dark mode** (token-driven). | ✅ (legacy lab pages: light only — follow-up) |
